# Healthcare Payments  and claims on Blockchain
...
Detailed instructions for installing on Ubuntu can be found here: https://
...
You will need two different identities. One can be shared between the Payer and Fractal Med (working on behalf of the patients) and the other one for a provider. The reason for this is that some data is stored only accessible to some identities (look for Private Collections later in this document), therefore a switch in the identity is made.
